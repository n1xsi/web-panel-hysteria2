...
Если у вас на сервере стоит базовый протокол **Hysteria2** без панели, то скорее всего ваш путь выдачи конфига выглядит так:
...
С данной же web-панелью ваш путь выдачи конфига будет таким:
1) Заходите на сайт;
2) Вписываете имя конфига и нажимаете кнопку "Создать";
3) Копируете готовый конфиг.
...
Настройте службу, чтобы панель работала всегда и запускалась после перезагрузки сервера:
```bash
